...
- `cargo xtask install` to install discogs-load as a binary in ~/.cargo/bin
...
fn run_dist() -> Result<()> {
    ...
    let src = Path::new("target")
        .join(&target)
        .join("release")
        .join(format!("discogs-load{}", suffix));
    let dst = Path::new("dist").join(format!("discogs-load-{}{}", target, suffix));
    ...
    match env::var("DIST_TARGET") {
        ...
        _ => {
            if cfg!(target_os = "linux") {
                ...
                panic!("Unsupported OS, maybe try setting DISCOGS-LOAD_TARGET")
            }
        }
    }
}
